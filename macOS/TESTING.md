...
Release verification builds once, runs `test.sh all` once and one deep bundle check, plus applicable release-helper fixtures. It prints the same change-based human checklist and freezes the verified installer/icon for packaging. Automated verification and packaging can complete while human checks are pending. The release skill requires explicit user results before public publication for affected interaction, recorded in the existing `build/release-notes.md` with version, scope, outcome and commit. Later changes to those behaviors or delivery artifacts invalidate that acceptance. Internal-only changes do not mechanically require typing acceptance.
