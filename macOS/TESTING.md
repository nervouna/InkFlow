...
| Automatic application updates | `UpdateTests` through `settings` covers default-off preferences, 24-hour scheduling, automatic-download orchestration, release/asset validation, bounded download/cache replacement and synthetic installer bundle selection | `SettingsUITests.checkUpdateSettings` exercises native toggles and their dependency without networking | Enable the controls, observe a newer signed release download and confirm the installer opens without installing automatically |
