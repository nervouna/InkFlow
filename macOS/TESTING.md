...
`TestSupport.swift` selects its direct non-GUI consumers rather than unrelated source/store, installer or workflow suites. `test-test-affected.sh` locks that dependency list; update the mapping and fixture whenever a test starts or stops importing the shared support module. `Package.swift` remains full-suite because it can alter any product or target graph. `EngineTests.swift` remains the full `engine` parent because all five engine units share that source file and a path-only selector cannot safely infer which scenario changed.
