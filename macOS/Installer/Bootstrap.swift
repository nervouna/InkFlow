...
package enum IFInstallerBootstrap {
    @MainActor package static func run(arguments: [String]) {
        if arguments.dropFirst() == ["--check-payload"] {
            Task.detached {
        ...
        let controller = IFInstallWindowController(version: version, installedVersion: existing, target: target,
            makeCoordinator: { .production(candidate: try await payload.load()) },
            cleanup: { try await payload.clean() })
