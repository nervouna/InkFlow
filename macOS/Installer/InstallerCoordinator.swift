...
typealias IFInstallerInputSourceOperations = IFPackageInputSourceOperations
typealias IFInstallerSystemInputSources = IFPackageSystemInputSources
typealias IFInstallerInputIdentity = IFPackageInputIdentity
#else
typealias IFInstallerInputSourceOperations = IFInputSourceOperations
typealias IFInstallerSystemInputSources = IFSystemInputSources
typealias IFInstallerInputIdentity = IFInputIdentity
#endif
...
@MainActor final class IFInstallerCoordinator {
    ...
    private let sources: any IFInstallerInputSourceOperations
    ...
    init(files: any IFInstallerFileOperations, sources: any IFInstallerInputSourceOperations,
    ...
    static func production(candidate: IFUnpackedShippedCandidate) -> IFInstallerCoordinator {
        .init(files: IFInstallerFiles(candidate: candidate), sources: IFInstallerSystemInputSources(), lifecycle: IFSystemLifecycle())
    }
    ...
    private func switchAwayIfSelected() {
        guard let roster = try? sources.snapshot(),
              [IFInstallerInputIdentity.bundleID, IFInstallerInputIdentity.modeID].contains(roster.selectedID),
              let fallback = roster.enabled.first(where: {
                  $0.bundleID != IFInstallerInputIdentity.bundleID && $0.enabled && $0.selectable && $0.ascii
              }) else { return }
        try? sources.select(fallback.id)
    }
    private func activate() async throws {
        state = .activating
        let ids = [IFInstallerInputIdentity.bundleID, IFInstallerInputIdentity.modeID]
        ...
        try sources.select(IFInstallerInputIdentity.modeID)
        for _ in 0..<8 {
            roster = try sources.snapshot()
            if roster.selectedID == IFInstallerInputIdentity.modeID { state = .installedEnabled; return }
            try await pause()
        }
