...
test_all_units=(quality-identity quality-store quality-timing quality-metadata quality-capture-query voice-session apple-voice voice-lexicon voice-controller ai-credentials ai-transport ai-runtime ai-statistics ai-learning ai-headless preparation dictionary-generator deployment engine-basic engine-options engine-english engine-context engine-custom-phrases controller settings dictionary-source dictionary-store dictionary-worker dictionary-activation startup-diagnostics local-diagnostics diagnostic-archive termination installer-core runner workflow)
expand_test_groups() {
  test_units=()
