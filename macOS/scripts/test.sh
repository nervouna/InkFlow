...
set -euo pipefail
cd "$(dirname "$0")/../.."
groups=(quality preparation dictionary-generator deployment engine controller settings dictionary-updates dictionary-activation)
usage() {
...
if [[ $# -eq 0 || ( $# -eq 1 && "$1" == all ) ]]; then
  ...
  for group in "$@"; do
    case "$group" in
      quality|preparation|dictionary-generator|deployment|engine|controller|settings|dictionary-updates|dictionary-activation) ;;
