...
enum IFDictionaryGenerator {
    ...
    private static func parse(_ data: Data, source: String, defaultWeight: Int?) throws -> [Row] {
        var rows = [Row]()
        try readRows(data, source: source, defaultWeight: defaultWeight) { rows.append($0) }
        return rows
    }

    static func spellingSyllables(in dictionary: Data) throws -> Set<String> {
        var syllables = Set<String>()
        try readRows(dictionary, source: "generated-spelling", defaultWeight: nil) { row in
            syllables.formUnion(row.key.reading.split(separator: " ").map(String.init))
        }
        return syllables
    }

    private static func readRows(_ data: Data, source: String, defaultWeight: Int?, receive: (Row) -> Void) throws {
...
enum IFSpellingGenerator {
    static func generate(dictionary: Data) throws -> [String: Data] {
    ...
    static func write(dictionary: Data, to directory: URL) throws {
