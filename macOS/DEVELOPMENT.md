...
- `Settings.swift` contains validated defaults, a SwiftUI `NavigationSplitView`/`Form`, the About view, and a small AppKit window/hosting shell. `DictionarySettings.swift` renders the process-owned dictionary coordinator as a compact status, entry count, one contextual action and collapsed diagnostics. Window delegate closure clears presented errors without cancelling work. The existing defaults keys, values, and notification name remain unchanged.
- `UpdateCoordinator.swift` schedules an immediate due check, records each completed attempt, and waits 24 hours before the next one while automatic checks remain enabled; an interrupted attempt may retry after restart. `UpdateService.swift` reads the latest non-draft, non-prerelease GitHub Release, accepts one version-matched arm64 DMG, bounds the download, and retains only that installer in `~/Library/Caches/InkFlow/Updates/`. `UpdateInstallerLauncher.swift` mounts the DMG read-only and opens its identity- and version-matched installer. Checks and downloads are default-off, run asynchronously outside input callbacks, and never install without the installer's user action.
...
- `scripts/prepare-rime.sh` copies pinned dictionaries and Lua modules and generates the supplemental mixed dictionary for every build and engine-test entry point. Lua supplies translations, bounded native metadata access, and the existing adoption-learning bridge; Rime retains editing, selection, and paging. See `DEPENDENCIES.md` for weighting and lookup boundaries.
