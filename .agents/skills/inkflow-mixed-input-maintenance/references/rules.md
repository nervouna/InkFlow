...
13. **Coexistence with other features.** The primary Pinyin schema also has bounded fuzzy spelling aliases; the independently compiled English dictionary keeps literal lookup and the mixed schema keeps its own algebra. OpenCC emoji suggestions and deduplication run after translation. Exact custom phrases retain their explicit priority and bypass preceding-text reranking; user-authored phrase text is outside the automatic English dictionary gate. The native app's preceding-text ranker promotes only eligible all-Han candidates on the current page, leaving relative English frequency order intact. These features do not add contextual English prediction. See `macOS/DEVELOPMENT.md` for their separate boundaries and `englishFeatureCoexistence()` in `EngineTests.swift` for integration checks.
