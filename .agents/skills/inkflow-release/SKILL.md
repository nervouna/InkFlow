...
Confirm the semantic version matches the selected bump. The source plist build is a minimum baseline; every `build.sh` invocation allocates a new build above the shared local counter and baseline, without changing tracked source. After verification, replace the provisional `build` with `appBuild` from `build/release-verification/installer.plist` and record that actual artifact build in internal notes and the required metadata table. Packaging, installer assembly, and the runner use this verified artifact build. Do not rebuild after prepare: retain the root verification receipt with the prepared attempt for recovery. Do not separately repeat the core profile, deep bundle check, or package fixtures: packaging performs repeatable fast structural checks against the exact candidate. Signing, notarization, Gatekeeper, and downloaded-asset checks still run on the release bytes below. Automated checks do not establish installation or upgrade acceptance when that check is required.
