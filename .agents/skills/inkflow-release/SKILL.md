...
Download both draft assets to a fresh temporary directory using `gh release download`. Compare them byte-for-byte with the local files and run `shasum -a 256 -c SHA256SUMS` there. Before `--draft=false`, check any change-based installation checklist against explicit user results in `build/release-notes.md`. Missing or failed required installation results block public publication: ask for those concrete pending results, keeping the prepared draft intact. Reuse confirmed results only while the delivery behavior and artifacts remain unchanged; later changes to either invalidate that acceptance and require renewed human verification. Do not create another persistent acceptance system or treat a successful automated receipt as human evidence. Input and Settings interaction are preaccepted before this workflow and are not publication gates. Only then publish:
